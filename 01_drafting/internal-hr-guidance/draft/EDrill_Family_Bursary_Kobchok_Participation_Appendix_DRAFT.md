# The EDrill Family Bursary - Labour-Hire Agency Participation Appendix

Status: Draft for internal review only  
Version: 0.1  
Important: this appendix reflects the approved Phase 1 internal labour-hire participation model. It remains in draft form and must not be treated as operational until the appointed labour-hire agency acknowledgement and relevant launch checklist items are complete.

Note: despite the current file name, this appendix is agency-neutral and is not limited to Kobchok. It applies only if the appointed labour-hire agency participation model and acknowledgement are approved.

## 1. Purpose

This Appendix defines how eligible employees of the appointed labour-hire agency at the time of application who are assigned to EDrill rigs may be considered for The EDrill Family Bursary.

The purpose is to support eligible children of workers contributing to EDrill rig operations while preserving the correct relationship between EDrill, each labour-hire agency, and the agency's employees.

## 2. Core Relationship Principle

Labour-hire agency employees are not to be described as EDrill employees unless that is legally accurate.

Approved wording:

> eligible employees of the appointed labour-hire agency at the time of application who are assigned to EDrill rigs

or:

> eligible employees of EDrill's contractor working on EDrill rigs

Avoid:

- "our employees" when referring to labour-hire agency staff;
- wording that implies employment by EDrill;
- public wording that blurs the contractor relationship.

## 3. Provisional Eligibility

A labour-hire agency employee may be considered if:

1. The relevant labour-hire agency confirms the individual is its employee.
2. EDrill Operations confirms the individual is assigned to EDrill rigs.
3. EDrill Operations and/or the relevant labour-hire agency confirms continuous work for EDrill since 1 January 2025.
4. The child meets age, relationship, schooling, and evidence rules.
5. Required consents and evidence are provided.
6. The approved Phase 1 agency payment route is used.
7. Chairman/Committee internal compliance controls are followed.

References to agency records in this Appendix mean the relevant labour-hire agency records.

## 4. Award Value

The provisional labour-hire award value is up to THB 8,000 per month.

The provisional payment cadence is quarterly, up to THB 24,000 per quarter.

Finance handles cost centre / GL code as an administrative processing detail. The approved payment route is EDMPL payment to the appointed agency for onward payment to its employee.

## 5. Verification Requirements

| Requirement | Verifier | Evidence / record |
| --- | --- | --- |
| Labour-hire agency employment status | Relevant labour-hire agency | Written confirmation |
| Assignment to EDrill rig | EDrill Operations | Assignment record or written confirmation |
| Continuous work since 1 Jan 2025 | EDrill Operations / labour-hire agency | Service or assignment confirmation |
| Child eligibility | HR | Application evidence |
| School eligibility and fees | HR / Finance | School evidence |
| Payment route | Finance | EDMPL payment to appointed agency for onward payment |

## 6. Labour-Hire Agency Acknowledgement

Before including labour-hire agency employees, EDrill should obtain practical written acknowledgement from the relevant agency confirming:

- the agency is aware of the Bursary;
- the agency will verify employment status where requested;
- the agency will support assignment/service verification where appropriate;
- EDMPL will pay any approved bursary funds to the agency for onward payment to the relevant employee;
- the agency will pay onward to its employee using its existing payroll/payment process, similar to salary handling;
- the agency is responsible for obtaining or holding employee receipt or acknowledgement of funds in the same practical way it handles salary/payment acknowledgement;
- EDrill may request confirmation that onward payment was made;
- participation does not alter the employment relationship;
- EDrill's support does not make agency employees EDrill employees;
- any data shared will be handled according to approved privacy requirements.

## 7. Payment Controls

Approved working payment route:

1. EDMPL pays the appointed labour-hire agency at the time of application quarterly.
2. The labour-hire agency pays the eligible employee using its existing payroll/payment process.
3. The labour-hire agency obtains or holds employee receipt or acknowledgement of funds in the same practical way it handles salary/payment acknowledgement.
4. EDrill may request confirmation that onward payment was made.

Do not create a burdensome new process unless the Committee or Finance later requires it. EDrill should not make direct payment to a labour-hire employee unless the Committee and Finance approve that route separately.

## 8. Records and Reporting

Labour-hire applications and awards should be identified separately in internal records and annual reporting.

Records should include:

- labour-hire agency employment verification;
- EDrill assignment verification;
- continuous work verification;
- application evidence;
- award approval;
- payment evidence;
- agency onward-payment confirmation, if requested;
- any labour-hire agency acknowledgement.

## 9. Communications

Internal and external communications should refer to labour-hire agency personnel carefully.

Suggested internal wording:

> The Bursary may also include eligible employees of the appointed labour-hire agency at the time of application who are assigned to EDrill rigs, subject to the approved labour-hire participation rules and verification requirements.

Suggested external wording, if a labour-hire agency is mentioned at all:

> The initiative supports eligible families connected with our Thailand operations, including approved contractor personnel assigned to EDrill rigs.

Do not publicly name any labour-hire agency unless EDrill and that agency have approved the wording.

Communications must contain no fundraising language, must not describe the Bursary as a charity, society, foundation, or ROS body, must not claim tax-free or non-taxable treatment, must not imply guaranteed entitlement, and must not identify children/beneficiaries without specific consent.

## 10. Approval

| Role | Name | Signature / approval record | Date |
| --- | --- | --- | --- |
| EDrill Executive Sponsor |  |  |  |
| EDrill Operations |  |  |  |
| EDrill Finance |  |  |  |
| Chairman / Committee internal compliance |  |  |  |
| Labour-hire agency representative, if required |  |  |  |
